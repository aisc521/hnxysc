        ...
        if (oddsCollection.length == 5) {
            ...
            for (int i = 0; i < BigMin.length; i++) {
                if (BigMin[i].split(",")[8].equals("1")) {
                    ...
                    if (scoreDb == null || scoreDb.getOddsid() == null || scoreDb.getOddsid() < 0){
                        ...
                        //对比即时赔率变化了没有
                        if (scoreDb.oddsEquals(score))bmJump++;else {
                            //变化了 更新
                    ...
                    if (db == null || db.getOddsid() == null || db.getOddsid() < 0){
                        ...
                        //对比即时赔率变化了没有
                        if (db.oddsEquals(xml))mBmJump++;else {
                            //变化了 更新
            ...
            for (int i = 0; i < halfLetBall.length; i++) {
                if (halfLetBall[i].split(",")[9].equals("1")){//单盘口
                    ...
                    if (db!=null && db.getOddsid()!=null && db.getOddsid()>0){
                        //检查是否需要更新
                        if (hlb.oddsEquals(db)) halfLetGoalJump++;else {
                    ...
                    }else {if (letGoalhalfMapper.insertSelective(hlb)>0) halfLetGoalInsert++;}
                }else {
                    //多盘
                    ...
                    if (db!=null && db.getOddsid()!=null && db.getOddsid()>0){
                        //检查是否需要更新
                        if (hlb.oddsEquals(db)) mHalfLetGoalJump++;else {
                    ...
                    }else {if (multiLetGoalhalfMapper.insertSelective(hlb)>0) mHalfLetGoalInsert++;}
                }
            }
                ...
                if (halfBigMin[i].split(",")[8].equals("1")){
                    ...
                    if(db!=null && db.getOddsid()!=null && db.getOddsid()>0){
                        ...
                        if (!db.oddsEquals(mo)){
                            ...
                            toDb.setUpodds(db.getUpodds());
                            toDb.setDownodds(db.getDownodds());
                            if (multiTotalScorehalfMapper.updateByPrimaryKeySelective(toDb)>0)mTotalScoreHalfUpdate++;
