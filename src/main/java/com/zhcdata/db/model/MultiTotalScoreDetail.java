...
public class MultiTotalScoreDetail {
    private Integer id;
    ...
    public boolean oddsEquals(MultiTotalScoreDetail xml) {
        try {

            return xml.getUpodds().equals(upodds) && xml.getDownodds().equals(downodds) && xml.getGoal().equals(goal) && xml.getAddtime().getTime() == addtime.getTime();
        }catch (Exception e){
            return true;
        }
    }
